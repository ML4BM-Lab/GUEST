...
def test(data_generator, model):
    ...
    return roc_auc_score(y_label, y_pred), average_precision_score(y_label, y_pred), f1_score(y_label,
                                                                                              outputs), y_pred, loss.item(), thred_optim, y_pred_s
...
def main():
    ...
    # normal run is:
    path_files = os.path.join('SplitsClassyfire_DrugBank', f'seed_{nseed}', f'fold{nfold}')

    df_train = pd.read_csv(path_files + '/train.csv')
    ...
    with torch.set_grad_enabled(False):
        auc, auprc, f1, logits, loss, _, _ = test(testing_generator, model_max)
    ...
    for epo in range(args.epochs):
        ...
        # every epoch test
        with torch.set_grad_enabled(False):
            auc, auprc, f1, logits, loss, _, _ = test(validation_generator, model)
            if auc > max_auc:
                model_max = copy.deepcopy(model)
                max_auc = auc
    ...
    try:
        with torch.set_grad_enabled(False):
            auc, auprc, f1, logits, loss, optimal_threshold, y_pred_after_threshold = test(testing_generator, model_max)
            print(
                'Testing AUROC: ' + str(auc) + ' , AUPRC: ' + str(auprc) + ' , F1: ' + str(f1) + ' , Test loss: ' + str(
                    loss))
            # save predictions 
            fpath =  os.path.join('predictions', f'seed_{nseed}')
            threshold = 'threshold=' + str(optimal_threshold) 
            d = {'y_pred': logits, 
                 threshold: y_pred_after_threshold}
            df_predictions = pd.DataFrame(d)
            fpath = fpath + f'/predictions_fold{nfold}.csv'
            df_predictions.to_csv(fpath)
